...
- 2026-06-12 (deploy): Merged to main (a37fde0). Droplet backend deployed and healthy (postgres, 1037 professors). Vercel's GitHub webhook had silently died (no deployment for the push; dashboard redeploy only rebuilt the old commit) — reconnected the git integration via CLI; this commit validates the restored auto-deploy path.
- 2026-06-13 (ghost removal): Deleted two superseded scan generations. **Gen-2 file-backed agentic** (`AgenticOnboardingService` class + all `/agentic/*` routes; kept shared `_call_llm`) and **Gen-1 adapter generation** (`execute_agentic_onboarding` + `agentic_scraper_service.py`) removed. Also deleted orphan `api/matches.py` (compat wrapper), the unused `/scrape-runs` subsystem (`api/scrape_runs.py` + `scrape_run_service.py` + router include), the dead `/scan-results/{id}/import` and `/scan-jobs/{id}/revise-publications` routes, the `revise_*` openalex aliases + unreachable return. Frontend: removed legacy job-viewer + recent-jobs panel from `/admin/onboarding` (now just the durable create form), the stale "Agentic scan dashboard" tabs from `/admin`, 13 dead `lib/api.ts` fns, and 2 orphan types. API surface 64→53 routes. Kept Gen-1 QA-artifact scans (`/scans*`, admin_scan_service, scraper adapters) — still covered by the `/admin/scrapes` e2e test. 47 backend + 9 jest + tsc + build all green.
- 2026-06-13 (UI polish): Admin tables reworked onto a shared `.data-table` style (sticky header, consistent padding/borders/hover) across `/admin`, `/admin/scans`, `/admin/onboarding`. Post-login splash screen added to `/signin` + `/signup` (no more being stuck on the form after auth). Skeleton loaders unified: landing stat shimmer + new `CardListSkeleton` replacing plain-text "Loading…" on `/universities`, `/universities/[slug]`, `/board`, `/compare`. OpenAlex confidence surfaced as a banded chip on durable scan candidates; legacy agentic enrich bumped 5→10 papers and now records `publication_confidence`. Durable scan path already fetched 10 papers with name+affiliation confidence. tsc + 9 jest + next build all green.
