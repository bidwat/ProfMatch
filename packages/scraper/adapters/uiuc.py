...
class UIUCAdapter(BaseUniversityAdapter):
    def __init__(self) -> None:
        super().__init__(
            university="University of Illinois Urbana-Champaign",
            department="Computer Science",
            faculty_roster_url="https://siebelschool.illinois.edu/about/people/all-faculty",
            adapter_name="uiuc",
        )

    async def _fetch_and_parse(self, url: str) -> list[tuple[str, str]]:
        """Fetch page with Crawl4AI and parse faculty from person cards."""
        ...
        # UIUC Siebel School: Find person cards
        person_cards = soup.find_all('div', class_=re.compile('person', re.I))
        ...
        for card in person_cards:
            ...
            for a in links:
                ...
                if not text or len(text) < 3 or len(text) > 50:
                    continue
                if any(kw in text.lower() for kw in ['view', 'read more', 'contact', 'email']):
                    continue
                ...
                if href.startswith('http'):
                    ...
                    full_url = f"https://siebelschool.illinois.edu{href}"
                else:
                    continue
    ...
    def scrape(self, *, run_id: str | None = None, output_root: Path | str = Path("."), fixture_path: Path | None = None, enrich_profiles: bool = True, enrich_publications: bool = False) -> RunOutputs:
        ...
        pages_attempted += 1
        print(f"Fetching UIUC Siebel School page with Crawl4AI: {self.faculty_roster_url}")
        try:
            ...
            print(f"Error fetching UIUC page: {e}")
