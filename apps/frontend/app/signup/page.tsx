    ...
    <div className="page narrow">
      <div className="card" style={{ maxWidth: 430, margin: '60px auto' }}>
        ...
        <form className="form" onSubmit={submit}>
          <label className="label">Full name<input className="input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Jordan Lee" /></label>
          ...
          <label className="label">Password<input className="input" type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="At least 8 characters" /></label>
          {error && <div className="error">{error}</div>}
          <button className="button primary" type="submit" disabled={submitting}>{submitting ? 'Creating account…' : 'Continue to profile →'}</button>
        </form>
